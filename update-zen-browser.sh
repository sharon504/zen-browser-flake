...
specfic=$(nix-prefetch-url --type sha256 --unpack $baseUrl/zen.linux-x86_64.tar.xz)
sed -i "s/specific.sha256 = \".*\"/specific.sha256 = \"$specfic\"/" ./flake.nix

# Update the hash generic.sha256
generic=$(nix-prefetch-url --type sha256 --unpack $baseUrl/zen.linux-x86_64.tar.xz)
